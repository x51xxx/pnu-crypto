import {gmul02} from "../gf";
import * as assert from "minimalistic-assert";

export class AES {
    private readonly NK = 4;
    private readonly NR = 10;
    ...
    private static coefMul(a: number[], b: number[], d: number[]) {
        d[0] = gmul02(a[0], b[0]) ^ gmul02(a[3], b[1]) ^ gmul02(a[2], b[2]) ^ gmul02(a[1], b[3]);
        d[1] = gmul02(a[1], b[0]) ^ gmul02(a[0], b[1]) ^ gmul02(a[3], b[2]) ^ gmul02(a[2], b[3]);
        d[2] = gmul02(a[2], b[0]) ^ gmul02(a[1], b[1]) ^ gmul02(a[0], b[2]) ^ gmul02(a[3], b[3]);
        d[3] = gmul02(a[3], b[0]) ^ gmul02(a[2], b[1]) ^ gmul02(a[1], b[2]) ^ gmul02(a[0], b[3]);
    }

    constructor(key: Uint8Array) {
        assert.equal(key.length, 16, 'Invalid key length(supports only 128 bites)');
    ...
    protected ecbEncrypt(cipher: AES, m: Uint8Array) {
        m = AES.pad(m);
    ...
    protected ecbDecrypt(cipher: AES, m: Uint8Array) {
        if (m.length % AES.blockSize !== 0) { throw 'Message is not properly padded'; }
        const output = new Uint8Array(m.length);
        for (let i = 0; i < m.length; i += AES.blockSize) {
    ...
    public encString(str: string) {
        const outBytes = this.ecbEncrypt(this, AES.computeMessage(str));
        return AES.hex(outBytes);
    }

    public decString(str: string) {
        const outBytes = this.ecbDecrypt(this, AES.str2hex(str));
    ...
    private static roundConstant(i: number) {
        if (i == 1) {
            ...
            i--;
            while (i - 1 > 0) {
                AES.AES_R[0] = gmul02(AES.AES_R[0], 0x02);
                i--;
            }
        }
